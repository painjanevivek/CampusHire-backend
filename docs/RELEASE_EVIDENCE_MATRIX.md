...
| Candidate freeze and launch | release-manifest generator, locally verified image/config/migration/rollback artifact pack, no-go dossier, deployment/rollback runbook, controlled launch record | frontend candidate SHA, immutable image/archive digest, and browser/accessibility evidence | clean backend worktree, approved registry promotion/signing, staging access, named go/no-go, and monitored deployment |
