...
| NFR reliability and recovery | `scripts/rehearse_postgres_recovery.ps1`, timed PostgreSQL 16 rollback/restore report, job lease/retry tests, provider fakes, and the Phase 7C production-like topology/rehearsal | contract check, standalone image, release smoke, rollback guide, and failure-state tests | managed staging restore and vendor worker/dependency drills |
